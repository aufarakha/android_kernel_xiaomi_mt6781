...
static int get_svp_memory_info(void)
{
	...
	if (!node) {
		pr_debug("%s, svp no use static reserved memory\n", __func__);
		return 1;
	}
...
static void  setup_feature_size(void)
{
	...
	if (!dt_node)
		pr_debug("%s, failed to find the ssmr device tree\n",
							__func__);
...
static int get_reserved_cma_memory(struct device *dev)
{
	...
	if (!rmem) {
		pr_debug("%s, no ssmr device info\n", __func__);
		return -EINVAL;
	}

	pr_debug("resource base=%pa, size=%pa\n", &rmem->base, &rmem->size);
				 ...
				 unsigned long *size, u64 upper_limit)
{
	...
	if (feature->cache_page) {
		...
		pr_debug("%s: [cache memory] pa %pa(%zx) is allocated\n",
	...
	pr_debug("%s[%d]: upper_limit: %llx, feature{ alloc_size : %lu",
				  ...
				  u64 upper_limit, unsigned int feat)
{
	...
	pr_debug("%s %d: [reserve done]: pa: %pad, size: 0x%lx\n", __func__,
...
static int memory_region_online(struct SSMR_Feature *feature)
{
	...
	if (feature->is_unmapping) {
		...
		ret_map = memory_mapping((unsigned long)
			...
			alloc_size);
		if (ret_map < 0)
			pr_debug("[remapping fail]: virt:0x%lx, size:0x%lx",
			(unsigned long)
			phys_to_virt(dma_to_phys(ssmr_dev, feature->phy_addr)),
			alloc_size);
		else
...
static int memory_ssmr_sysfs_init(void)
{
	...
	if (ssmr_kobject) {
		...
		pr_debug("SSMR: Cannot find module %s object\n",
				KBUILD_MODNAME);
		return -EINVAL;
	}
...
int ssmr_probe(struct platform_device *pdev)
{
	...
	pr_debug("memory_ssmr driver probe done\n");
