...
#define PWR_LOG_PM(format, args...) \
	do { \
		if (g_pwr_log_level >= APUSYS_PWR_LOG_WARN) \
			pr_debug(DVFS_TAG "[pm] " format, ##args); \
	} while (0)

#define PWR_LOG_ERR(format, args...) \
		pr_debug(DVFS_TAG "[error] " format, ##args)
...
#define LOG_PM(format, args...) \
	do { \
		if (g_pwr_log_level >= APUSYS_PWR_LOG_WARN) \
			pr_debug(PWR_TAG "[pm] " format, ##args); \
	} while (0)

#define LOG_INF(format, args...) \
	do { \
		if (g_pwr_log_level >= APUSYS_PWR_LOG_INFO) \
			pr_debug(PWR_TAG " " format, ##args); \
	} while (0)
...
#define LOG_ERR(format, args...)    pr_debug(PWR_TAG "[error] " format, ##args)
