...
unsigned int get_dram_data_rate(void)
{
	unsigned int u4PllIdx, u4ShuLevel, u4SDM_PCW, u4PREDIV, u4POSDIV,
				 ...
				 u4ARDQ_REV_B0, u4PhyPLLDDR400EN, u4CKMUL2;
	...
	u4CKMUL2 = readl(
		IOMEM(DDRPHY_AO_CHA_BASE_ADDR + 0xd90 +
			(0x500 * u4ShuLevel))) & 0x0000ffff;
...
DRIVER_ATTR(read_mr4, 0664, read_mr4_show, read_mr4_store);
#endif
...
{
	//dramc_info("apxgpt3 irq handled");
...
static int dramc_apxgpt3_init(void)
{
	...
	ret = request_irq(gpt_timers.tmr_irq, dramc_apxgpt3_irq_handler,
			IRQF_TIMER | IRQF_IRQPOLL | IRQF_TRIGGER_LOW,
				"dramc_apxgpt3", NULL);
	if (ret) {
		dramc_info("apxgpt3 request irq Failed\n");
		return -1;
	}

//	dramc_info("enable apxgpt3 one-shot");
