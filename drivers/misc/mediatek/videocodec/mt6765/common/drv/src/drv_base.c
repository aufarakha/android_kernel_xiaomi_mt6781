						...
						unsigned int tid)
{
	...
	pr_debug("[VCODEC] %s All %d Slots unavaliable\n",
	...
	unsigned int *a_prIndex)
{
	...
	{
		pr_debug("%s All %d Slots unavaliable\n",
				__func__, VCODEC_INST_NUM);
			    ...
			    unsigned int *a_pu4VCodecThreadID)
{
	...
	if (u4I == VCODEC_INST_NUM_x_10) {
		 ...
		 *  WARNING:BRACES:
		 */
		pr_debug("CAN'T ADD %s, List is FULL!!\n", __func__);
	}
...
void free_ncmem(unsigned long a_ulKVA, unsigned long a_ulKPA)
{
	...
	if (u4I == VCODEC_INST_NUM_x_10) {
		 ...
		 * WARNING:BRACES:
		 */
		pr_debug("CAN'T Free %s, Address is not find!!\n", __func__);
	}
...
unsigned long search_ncmem_byKPA(unsigned long a_ulKPA)
{
	...
	if (u4I == VCODEC_INST_NUM_x_10) {
		pr_debug("CAN'T Find address %s", __func__);
