...
static int _mt6779_ipi_sspm_send(int flags, unsigned int cmd)
{
	...
	if ((flags == SSPM_NOTIFY_ENTER_ASYNC)
	   || (flags == SSPM_NOTIFY_LEAVE_ASYNC)) {
		...
		if (ret != 0)
			pr_debug("[name:spm&]#@# %s(%d) async(cmd:0x%x) ret %d\n",
		...
		if (ret != 0)
			pr_debug("[name:spm&]#@# %s(%d) sync(cmd:0x%x) ret %d\n",
					__func__, __LINE__, spm_d.cmd, ret);
		else if (ack_data < 0) {
			ret = ack_data;
			pr_debug("[name:spm&]#@# %s(%d) sync cmd:0x%x ret %d\n",
...
#else
	pr_debug("[name:spm&]#@# %s(%d) TINYSYS_SSPM_SUPPORT is not support\n",
			__func__, __LINE__);
...
static int _mt6779_ipi_sspm_response(int flags, unsigned int cmd)
{
	...
	if (ret != 0)
		pr_debug("[name:spm&]#@# %s(%d) async_wait(cmd:0x%x) ret %d\n",
				__func__, __LINE__, cmd, ret);
	else if (ack_data < 0) {
		ret = ack_data;
		pr_debug("[name:spm&]#@# %s(%d) async_waitcmd(%d) return %d\n",
			__func__, __LINE__, cmd, ret);
	}
#else
	pr_debug("[name:spm&]#@# %s(%d) TINYSYS_SSPM_SUPPORT is not support\n",
