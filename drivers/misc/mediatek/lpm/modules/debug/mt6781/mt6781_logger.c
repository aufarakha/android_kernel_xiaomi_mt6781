...
#define MT6781_LOG_MONITOR_STATE_NAME	"mcusysoff"
#define MT6781_LOG_DEFAULT_MS		5000
...
struct MT6886_LOGGER_NODE mt6781_log_gs_idle;
struct MTK_LPM_GS_IDLE_INFO {
	unsigned short limit;
	unsigned short limit_set;
	unsigned int dump_type;
};
struct MTK_LPM_GS_IDLE_INFO mt6781_log_gs_info;
...
static struct mt6781_spm_wake_status mt6781_wake;
void __iomem *mt6781_spm_base;

struct mt6781_log_helper {
	short cur;
	short prev;
	struct mt6781_spm_wake_status *wakesrc;
};
struct mt6781_log_helper mt6781_logger_help = {
	.wakesrc = &mt6781_wake,
	.cur = 0,
	.prev = 0,
};
...
const char *mt6781_wakesrc_str[32] = {
	[0] = " R12_PCM_TIMER",
	[1] = " R12_TWAM_IRQ_B",
	[2] = " R12_KP_IRQ_B",
	[3] = " R12_APWDT_EVENT_B",
...
struct spm_wakesrc_irq_list mt6781_spm_wakesrc_irqs[] = {
	/* mtk-kpd */
	...
	{ WAKE_SRC_STA1_CONN2AP_SPM_WAKEUP_B, "mediatek,mt6781-consys", 0, 0},
	/* conn2ap_sw_irq */
...
#define plat_mmio_read(offset)	__raw_readl(mt6781_spm_base + offset)
u64 ap_pd_count;
u64 ap_slp_duration;
...
#define IRQ_NUMBER	\
(sizeof(mt6781_spm_wakesrc_irqs)/sizeof(struct spm_wakesrc_irq_list))
static void mt6781_get_spm_wakesrc_irq(void)
{
	int i;
	struct device_node *node = NULL;

	for (i = 0; i < IRQ_NUMBER; i++) {
		if (mt6781_spm_wakesrc_irqs[i].name == NULL)
			continue;

		node = of_find_compatible_node(NULL, NULL,
			mt6781_spm_wakesrc_irqs[i].name);
		if (!node) {
		...
		mt6781_spm_wakesrc_irqs[i].irq_no =
			irq_of_parse_and_map(node,
				mt6781_spm_wakesrc_irqs[i].order);

		if (!mt6781_spm_wakesrc_irqs[i].irq_no) {
...
struct mt6781_logger_timer {
	struct mt_lpm_timer tm;
	unsigned int fired;
};
struct mt6781_logger_fired_info {
	unsigned int fired;
	int state_index;
};

static struct mt6781_logger_timer mt6781_log_timer;
static struct mt6781_logger_fired_info mt6781_logger_fired;

int mt6781_get_wakeup_status(struct mt6781_log_helper *help)
{
	if (!help->wakesrc || !mt6781_spm_base)
		return -EINVAL;
...
static void mt6781_save_sleep_info(void)
{
...
static void mt6781_suspend_show_detailed_wakeup_reason
	(struct mt6781_spm_wake_status *wakesta)
{
	int i;
	unsigned int irq_no;
	...
	for (i = 0; i < IRQ_NUMBER; i++) {
		if (mt6781_spm_wakesrc_irqs[i].name == NULL ||
			!mt6781_spm_wakesrc_irqs[i].irq_no)
			continue;
		if (mt6781_spm_wakesrc_irqs[i].wakesrc & wakesta->r12) {
			irq_no = mt6781_spm_wakesrc_irqs[i].irq_no;
...
static void dump_lp_cond(void)
{
	...
	for (i = 1 ; i < PLAT_SPM_COND_MAX ; i++) {
		blkcg = MT6781_DBG_SMC(MT_SPM_DBG_SMC_UID_BLOCK_DETAIL, MT_LPM_SMC_ACT_GET, 0, i);
		if (blkcg != 0)
...
static void mt6781_suspend_spm_rsc_req_check
	(struct mt6781_spm_wake_status *wakesta)
{
...
static int mt6781_show_message(struct mt6781_spm_wake_status *wakesrc, int type,
...
#undef LOG_BUF_SIZE
	...
	if (wakesrc->is_abort != 0) {
		...
		for (i = 1; i < 32; i++) {
			if (wakesrc->r12 & (1U << i)) {
				if (IS_LOGBUF(buf, mt6781_wakesrc_str[i]))
					strncat(buf, mt6781_wakesrc_str[i],
						strlen(mt6781_wakesrc_str[i]));
	...
	if (type == MT_LPM_ISSUER_SUSPEND) {
		printk_deferred("[name:spm&][SPM] %s", log_buf);
		mt6781_suspend_show_detailed_wakeup_reason(wakesrc);
		mt6781_suspend_spm_rsc_req_check(wakesrc);
...
int mt6781_issuer_func(int type, const char *prefix, void *data)
{
	mt6781_get_wakeup_status(&mt6781_logger_help);
	return mt6781_show_message(mt6781_logger_help.wakesrc,
					type, prefix, data);
}

struct mtk_lpm_issuer mt6781_issuer = {
	.log = mt6781_issuer_func,
};

static int mt6781_idle_save_sleep_info_nb_func(struct notifier_block *nb,
			unsigned long action, void *data)
{
	struct mtk_lpm_nb_data *nb_data = (struct mtk_lpm_nb_data *)data;

	if (nb_data && (action == MTK_LPM_NB_BEFORE_REFLECT))
		mt6781_save_sleep_info();

	return NOTIFY_OK;
}

struct notifier_block mt6781_idle_save_sleep_info_nb = {
	.notifier_call = mt6781_idle_save_sleep_info_nb_func,
};

static void mt6781_suspend_save_sleep_info_func(void)
{
	mt6781_save_sleep_info();
}

static struct syscore_ops mt6781_suspend_save_sleep_info_syscore_ops = {
	.resume = mt6781_suspend_save_sleep_info_func,
};
static int mt6781_log_timer_func(unsigned long long dur, void *priv)
{
	struct mt6781_logger_timer *timer =
			(struct mt6781_logger_timer *)priv;
	struct mt6781_logger_fired_info *info = &mt6781_logger_fired;

	if (timer->fired != info->fired) {
		/* if the wake src had beed update before
		 * then won't do wake src update
		 */
		if (mt6781_logger_help.prev == mt6781_logger_help.cur)
			mt6781_get_wakeup_status(&mt6781_logger_help);
		mt6781_show_message(mt6781_logger_help.wakesrc,
					MT_LPM_ISSUER_CPUIDLE,
					"MCUSYSOFF", NULL);
		mt6781_logger_help.prev = mt6781_logger_help.cur;
	} else
...
static int mt6781_logger_nb_func(struct notifier_block *nb,
			unsigned long action, void *data)
{
	struct mtk_lpm_nb_data *nb_data = (struct mtk_lpm_nb_data *)data;
	struct mt6781_logger_fired_info *info = &mt6781_logger_fired;
	...
	if (nb_data && (action == MTK_LPM_NB_AFTER_PROMPT)
		...
		if (IS_MTK_LPM_GS_UPDATE(mt6781_log_gs_info)) {
			...
			mt6781_log_gs_info.limit =
				mt6781_log_gs_info.limit_set;
		}
	}
#endif
	return NOTIFY_OK;
...
struct notifier_block mt6781_logger_nb = {
	.notifier_call = mt6781_logger_nb_func,
};

static ssize_t mt6781_logger_debugfs_read(char *ToUserBuf,
					size_t sz, void *priv)
{
	char *p = ToUserBuf;
	int len;

	if (priv == ((void *)&mt6781_log_timer)) {
		len = scnprintf(p, sz, "%lu\n",
			mtk_lpm_timer_interval(&mt6781_log_timer.tm));
		p += len;
	}
#ifdef CONFIG_MTK_LPM_GS_DUMP_SUPPORT
	else if (priv == ((void *)&mt6781_log_gs_info)) {
		len = scnprintf(p, sz, "golden_type: %u\n",
				mt6781_log_gs_info.dump_type);
	}
#endif
	return (p - ToUserBuf);
}

static ssize_t mt6781_logger_debugfs_write(char *FromUserBuf,
				   size_t sz, void *priv)
{
	if (priv == ((void *)&mt6781_log_timer)) {
		unsigned int val = 0;
		...
		if (!kstrtouint(FromUserBuf, 10, &val)) {
			if (val == 0)
				mtk_lpm_timer_stop(&mt6781_log_timer.tm);
			else
				mtk_lpm_timer_interval_update(
						&mt6781_log_timer.tm, val);
		}
	}
#ifdef CONFIG_MTK_LPM_GS_DUMP_SUPPORT
	else if (priv == ((void *)&mt6781_log_gs_info)) {
		...
		if (sscanf(FromUserBuf, "%20s %u", cmd, &param) == 2) {
			if (!strcmp(cmd, "golden_dump")) {
				if (param)
					mt6781_log_gs_info.limit_set += 1;
			} else if (!strcmp(cmd, "golden_type"))
				mt6781_log_gs_info.dump_type = param;
		}
	}
#endif
	return sz;
...
#define MT6781_LOGGER_NODE_INIT(_n, _priv) ({\
	_n.op.fs_read = mt6781_logger_debugfs_read;\
	_n.op.fs_write = mt6781_logger_debugfs_write;\
	_n.op.priv = _priv; })\


struct mtk_lp_sysfs_handle mt6781_log_tm_node;
struct MT6886_LOGGER_NODE mt6781_log_tm_interval;


int mt6781_logger_timer_debugfs_init(void)
{
	mtk_lpm_sysfs_sub_entry_add("logger", 0644,
				NULL, &mt6781_log_tm_node);

	MT6781_LOGGER_NODE_INIT(mt6781_log_tm_interval,
				&mt6781_log_timer);
	mtk_lpm_sysfs_sub_entry_node_add("interval", 0644,
				&mt6781_log_tm_interval.op,
				&mt6781_log_tm_node,
				&mt6781_log_tm_interval.handle);

#ifdef CONFIG_MTK_LPM_GS_DUMP_SUPPORT
	MT6781_LOGGER_NODE_INIT(mt6781_log_gs_idle,
				&mt6781_log_gs_info);

	mtk_lpm_sysfs_sub_entry_node_add("gs", 0644,
				&mt6781_log_gs_idle.op,
				&mt6781_log_tm_node,
				&mt6781_log_gs_idle.handle);
#endif
	return 0;
}

int __init mt6781_logger_init(void)
{
	struct device_node *node = NULL;
	...
	if (node) {
		mt6781_spm_base = of_iomap(node, 0);
		of_node_put(node);
	}

	if (mt6781_spm_base)
		mtk_lp_issuer_register(&mt6781_issuer);
	else
		pr_debug("[name:mtk_lpm][P] - Don't register the issue by error! (%s:%d)\n",
			__func__, __LINE__);


	mt6781_get_spm_wakesrc_irq();
	mtk_lpm_sysfs_root_entry_create();
	mt6781_logger_timer_debugfs_init();
	...
	if (drv) {
		...
		for (idx = 0; idx < drv->state_count; ++idx) {
			if (!strcmp(MT6781_LOG_MONITOR_STATE_NAME,
					drv->states[idx].name)) {
				mt6781_logger_fired.state_index = idx;
				break;
			}
		}
	}

	mtk_lpm_notifier_register(&mt6781_logger_nb);
	mtk_lpm_notifier_register(&mt6781_idle_save_sleep_info_nb);

	mt6781_log_timer.tm.timeout = mt6781_log_timer_func;
	mt6781_log_timer.tm.priv = &mt6781_log_timer;
	mtk_lpm_timer_init(&mt6781_log_timer.tm, MTK_LPM_TIMER_REPEAT);
	mtk_lpm_timer_interval_update(&mt6781_log_timer.tm,
					MT6781_LOG_DEFAULT_MS);
	mtk_lpm_timer_start(&mt6781_log_timer.tm);

	register_syscore_ops(&mt6781_suspend_save_sleep_info_syscore_ops);

	return 0;
}
late_initcall_sync(mt6781_logger_init);
