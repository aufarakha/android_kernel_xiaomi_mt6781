...
int gps_pwr_msg_cb(enum conn_pwr_event_type event_type, void *level)
{
	...
	if (event_type == CONN_PWR_EVENT_LEVEL) {
		pr_debug("gps_pwr_msg_cb been called, level=%d, gps_pwr_open_state=%d\n",
...
static int gps_pwr_open(struct inode *inode, struct file *file)
{
	...
	has_new_battery_level = true;
	pr_debug("conn_pwr_drv_pre_on ret = %d, battery_level = %d\n", ret, battery_level);
	return 0;
}
...
static int gps_pwr_close(struct inode *inode, struct file *file)
{
	...
	pr_debug("conn_pwr_drv_post_off ret = %d\n", ret);
	return 0;
}
...
static ssize_t gps_pwr_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	pr_debug("gps_pwr read\n");

	if (count > sizeof(int))
		count = sizeof(int);

	if (copy_to_user(buf, (void *)&battery_level, sizeof(int))) {
		pr_debug("gps_pwr_read failed,because copy_to_user error\n");
		retval = -EFAULT;
	} else {
		retval = sizeof(int);
...
OUT:
	/*	pr_debug("gps_pwr_read(): retval = %d\n", retval);*/
		return retval;
...
static unsigned int gps_pwr_poll(struct file *file, poll_table *wait)
{
	...
	pr_debug("gps_pwr_poll been return , mask = %d\n", mask);
...
static int gps_pwr_init(void)
{
	...
	pr_debug("Registering gps_pwr chardev\n");
	...
	pr_debug("GPS_PWR device init Done\n");
	return 0;
...
static void gps_pwr_exit(void)
{
	...
	pr_debug("Unregistering gps_pwr chardev\n");
	cdev_del(&gps_pwr_devobj->chdev);
	unregister_chrdev_region(gps_pwr_devobj->devno, 1);
...
{
	pr_debug("gps_pwr init begin");
	return gps_pwr_init();
}

void mtk_gps_pwr_exit(void)
{
	pr_debug("gps_pwr exit begin");
	return gps_pwr_exit();
}
/*****************************************************************************/
