 ...
 *
 * This file, mt6781_pos_gen.c was automatically generated
...
void consys_set_if_pinmux_mt6781_gen(int enable)
{
	...
	void __iomem *vir_addr_consys_gen_iocfg_lt_base = NULL;

	vir_addr_consys_gen_gpio_base = ioremap(CONSYS_GEN_GPIO_BASE_ADDR, 0x480);
	vir_addr_consys_gen_iocfg_lt_base = ioremap(CONSYS_GEN_IOCFG_LT_BASE_ADDR, 0xC00);

	if (!vir_addr_consys_gen_gpio_base) {
		pr_err("vir_addr_consys_gen_gpio_base(%x) ioremap fail\n", CONSYS_GEN_GPIO_BASE_ADDR);
		return;
	}

	if (!vir_addr_consys_gen_iocfg_lt_base) {
		pr_err("vir_addr_consys_gen_iocfg_lt_base(%x) ioremap fail\n", CONSYS_GEN_IOCFG_LT_BASE_ADDR);
	...
	if (enable == 1) {
		...
		CONSYS_REG_WRITE_MASK(vir_addr_consys_gen_gpio_base +
						CONSYS_GEN_GPIO_MODE23_OFFSET_ADDR, 0x10000000, 0x70000000);

		/* set BT_SPI and TOP_SPI pinmux driving to 4mA */
		/* (TOP_SPI 8 mA for bring up ), */
		/* others keep default driving 2mA */
		CONSYS_REG_WRITE_MASK(vir_addr_consys_gen_iocfg_lt_base +
						0xC00, 0x1209, 0x7FFFFFF);

		/* set GPIO73 pinmux for TCXO mode (Aux2) */
		#if CLK_CTRL_TCXOENA_REQ
			if (wmt_plat_soc_co_clock_flag_get() == 0) {
				CONSYS_REG_WRITE_MASK(vir_addr_consys_gen_gpio_base +
								CONSYS_GEN_GPIO_MODE9_OFFSET_ADDR, 0x20, 0x70);
			}
		#endif
	} else {
		...
		CONSYS_REG_WRITE_MASK(vir_addr_consys_gen_gpio_base +
						CONSYS_GEN_GPIO_MODE23_OFFSET_ADDR, 0x0, 0x70000000);

		/* set GPIO73 pinmux for TCXO mode (Aux0) */
		#if CLK_CTRL_TCXOENA_REQ
			if (wmt_plat_soc_co_clock_flag_get() == 0) {
				CONSYS_REG_WRITE_MASK(vir_addr_consys_gen_gpio_base +
								CONSYS_GEN_GPIO_MODE9_OFFSET_ADDR, 0x0, 0x70);
			}
		#endif
	}
	...
	if (vir_addr_consys_gen_iocfg_lt_base)
		iounmap(vir_addr_consys_gen_iocfg_lt_base);
}

void consys_hw_reset_bit_set_mt6781_gen(int enable)
{
	if (conn_reg.ap_rgu_base == 0) {
		pr_err("conn_reg.ap_rgu_base is not defined\n");
		return;
	}
...
void consys_hw_spm_clk_gating_enable_mt6781_gen(void)
{
	if (conn_reg.spm_base == 0) {
		pr_err("conn_reg.spm_base is not defined\n");
		return;
	}
...
int consys_hw_power_ctrl_mt6781_gen(int enable)
{
	...
	if (conn_reg.topckgen_base == 0) {
		pr_err("conn_reg.topckgen_base is not defined\n");
		return -1;
	}
	...
	if (enable == 1) {
		...
		CONSYS_REG_BIT_POLLING(conn_reg.spm_base +
						...
						1, 1, 10, 500, check);
		if (check != 0)
			pr_err("check conn_top_on primary part power status fail, Status=0x%08x\n",
		...
		CONSYS_REG_BIT_POLLING(conn_reg.spm_base +
						...
						1, 1, 10, 500, check);
		if (check != 0)
			pr_err("check conn_top_on secondary part power status fail, Status=0x%08x\n",
				CONSYS_REG_READ(conn_reg.spm_base +
								CONSYS_GEN_PWR_STATUS_2ND_OFFSET_ADDR));
		...
		CONSYS_REG_WRITE(conn_reg.topckgen_base +
						CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_CLR_OFFSET_ADDR, 0x10000);
		...
		CONSYS_REG_BIT_POLLING(conn_reg.topckgen_base +
						...
						16, 0, 10, 500, check);
		if (check != 0)
			pr_err("check AHB RX bus sleep protect turn off fail, Status=0x%08x\n",
		...
		CONSYS_REG_WRITE(conn_reg.topckgen_base +
						CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_CLR_OFFSET_ADDR, 0x4000);
		...
		CONSYS_REG_BIT_POLLING(conn_reg.topckgen_base +
						...
						14, 0, 10, 500, check);
		if (check != 0)
			pr_err("check AXI Rx bus sleep protect turn off fail, Status=0x%08x\n",
				CONSYS_REG_READ(conn_reg.topckgen_base +
								CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_STA1_OFFSET_ADDR));
		...
		/* (disable sleep protection when CONNSYS had been turned on) */
		CONSYS_REG_WRITE(conn_reg.topckgen_base +
						CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_1_CLR_OFFSET_ADDR, 0x40000);
		...
		CONSYS_REG_BIT_POLLING(conn_reg.topckgen_base +
						...
						18, 0, 10, 500, check);
		if (check != 0)
			pr_err("check AXI Tx bus sleep protect turn off fail, Status=0x%08x\n",
				CONSYS_REG_READ(conn_reg.topckgen_base +
								CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_STA1_1_OFFSET_ADDR));
		...
		/* (apply this for INFRA AHB bus accessing when CONNSYS had been turned on) */
		CONSYS_REG_WRITE(conn_reg.topckgen_base +
						CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_CLR_OFFSET_ADDR, 0x2000);
		...
		CONSYS_REG_BIT_POLLING(conn_reg.topckgen_base +
						...
						13, 0, 10, 500, check);
		if (check != 0)
			pr_err("check AHB TX bus sleep protect turn off fail, Status=0x%08x\n",
				CONSYS_REG_READ(conn_reg.topckgen_base +
								CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_STA1_OFFSET_ADDR));
	} else {
		...
		CONSYS_REG_WRITE(conn_reg.topckgen_base +
						CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_SET_OFFSET_ADDR, 0x2000);
		...
		CONSYS_REG_BIT_POLLING(conn_reg.topckgen_base +
						...
						13, 1, 10, 1000, check);
		if (check != 0)
			pr_err("check AHB TX bus sleep protect turn on fail, Status=0x%08x\n",
				CONSYS_REG_READ(conn_reg.topckgen_base +
								CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_STA1_OFFSET_ADDR));
		...
		/* (apply this for INFRA AXI bus protection to prevent bus hang when CONNSYS had been turned off) */
		CONSYS_REG_WRITE(conn_reg.topckgen_base +
						CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_1_SET_OFFSET_ADDR, 0x40000);
		...
		CONSYS_REG_BIT_POLLING(conn_reg.topckgen_base +
						...
						18, 1, 100, 1000, check);
		if (check != 0)
			pr_err("check AXI Tx bus sleep protect turn on fail, Status=0x%08x\n",
				CONSYS_REG_READ(conn_reg.topckgen_base +
								CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_STA1_1_OFFSET_ADDR));
		...
		/* (apply this for INFRA AXI bus protection to prevent bus hang when CONNSYS had been turned off) */
		CONSYS_REG_WRITE(conn_reg.topckgen_base +
						CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_SET_OFFSET_ADDR, 0x4000);
		...
		CONSYS_REG_BIT_POLLING(conn_reg.topckgen_base +
						...
						14, 1, 100, 1000, check);
		if (check != 0)
			pr_err("check AXI Rx bus sleep protect turn on fail, Status=0x%08x\n",
				CONSYS_REG_READ(conn_reg.topckgen_base +
								CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_STA1_OFFSET_ADDR));
		...
		/* (apply this for INFRA AXI bus protection to prevent bus hang when CONNSYS had been turned off) */
		CONSYS_REG_WRITE(conn_reg.topckgen_base +
						CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_SET_OFFSET_ADDR, 0x10000);
		...
		CONSYS_REG_BIT_POLLING(conn_reg.topckgen_base +
						...
						16, 1, 10, 1000, check);
		if (check != 0)
			pr_err("check AHB RX bus sleep protect turn on fail, Status=0x%08x\n",
				CONSYS_REG_READ(conn_reg.topckgen_base +
								CONSYS_GEN_INFRA_TOPAXI_PROTECTEN_STA1_OFFSET_ADDR));
...
int polling_consys_chipid_mt6781_gen(unsigned int *pconsys_ver_id, unsigned int *pconsys_fw_ver)
{
	...
	if (conn_reg.mcu_top_misc_off_base == 0) {
		pr_err("conn_reg.mcu_top_misc_off_base is not defined\n");
		return -1;
	}

	if (conn_reg.mcu_base == 0) {
		pr_err("conn_reg.mcu_base is not defined\n");
		return -1;
	}
	...
	while (retry-- > 0) {
		...
		if (consys_ver_id == CONN_HW_VER) {
			check = 0;
			pr_debug("retry(%d)consys version id(0x%08x)\n", retry, consys_ver_id);
			if (pconsys_ver_id != NULL)
				*pconsys_ver_id = consys_ver_id;
			break;
		}
		check = -1;
		pr_debug("Read CONSYS version id(0x%08x)\n", consys_ver_id);
		msleep(20);
	}
	if (check != 0) {
		pr_err("[%s] Polling chip id fail (0x%x)\n", __func__, consys_ver_id);
		return -1;
	}
	...
	if (((check & 0xf) >> 0) != 0x2)
		pr_err("[%s] read CONNSYS configuration ID fail (0x%x)\n", __func__, check);
	else
	...
	if (((check & 0xffff) >> 0) != 0x8A00)
		pr_err("[%s] read CONNSYS HW version ID fail (0x%x)\n", __func__, check);
	else
	...
	if (((check & 0xffff) >> 0) != 0x8A00)
		pr_err("[%s] read CONNSYS FW version ID fail (0x%x)\n", __func__, check);
	else
...
void consys_set_access_emi_hw_mode_mt6781_gen(void)
{
	if (conn_reg.mcu_top_misc_on_base == 0) {
		pr_err("conn_reg.mcu_top_misc_on_base is not defined\n");
		return;
	}
...
void consys_afe_reg_setting_mt6781_gen(void)
{
	...
	if (!vir_addr_consys_gen_conn_afe_ctl_base) {
		pr_err("vir_addr_consys_gen_conn_afe_ctl_base(%x) ioremap fail\n", CONSYS_GEN_CONN_AFE_CTL_BASE_ADDR);
		return;
	}
...
void consys_set_xo_osc_ctrl_mt6781_gen(void)
{
	...
	if (consys_is_rc_mode_enable() == 0) {
		...
		if (wmt_plat_soc_co_clock_flag_get()) {
			...
			CONSYS_REG_WRITE(conn_reg.mcu_top_misc_on_base +
							CONSYS_GEN_CONN_ON_RC_OSC_CTL_GPS_0_OFFSET_ADDR, 0x2757473);
		}
		...
		if (wmt_plat_soc_co_clock_flag_get()) {
			...
			CONSYS_REG_WRITE(conn_reg.mcu_top_misc_on_base +
							CONSYS_GEN_CONN_ON_RC_OSC_CTL_BT_0_OFFSET_ADDR, 0x2757473);
		}
		...
		if (wmt_plat_soc_co_clock_flag_get()) {
			...
			CONSYS_REG_WRITE(conn_reg.mcu_top_misc_on_base +
							CONSYS_GEN_CONN_ON_RC_OSC_CTL_WF_0_OFFSET_ADDR, 0x2757473);
		}
		...
		if (wmt_plat_soc_co_clock_flag_get()) {
			...
			CONSYS_REG_WRITE(conn_reg.mcu_top_misc_on_base +
							CONSYS_GEN_CONN_ON_RC_OSC_CTL_TOP_0_OFFSET_ADDR, 0x2757473);
		}
		...
		CONSYS_REG_WRITE_MASK(conn_reg.mcu_top_misc_on_base +
						CONSYS_GEN_CONN_ON_RC_OSC_CTL_0_OFFSET_ADDR, 0x1010, 0x1010);

		/* enable conn_top rc osc_ctrl_bt */
		CONSYS_REG_WRITE_MASK(conn_reg.mcu_top_misc_on_base +
						CONSYS_GEN_CONN_ON_RC_OSC_CTL_0_OFFSET_ADDR, 0x2020, 0x2020);

		/* enable conn_top rc osc_ctrl_wf */
		CONSYS_REG_WRITE_MASK(conn_reg.mcu_top_misc_on_base +
						CONSYS_GEN_CONN_ON_RC_OSC_CTL_0_OFFSET_ADDR, 0x4040, 0x4040);

		/* enable conn_top rc osc_ctrl_top */
		CONSYS_REG_WRITE_MASK(conn_reg.mcu_top_misc_on_base +
						CONSYS_GEN_CONN_ON_RC_OSC_CTL_0_OFFSET_ADDR, 0x8080, 0x8080);

		/* disable legacy osc control */
		CONSYS_CLR_BIT(conn_reg.mcu_top_misc_on_base +
						CONSYS_GEN_CONN_ON_RC_OSC_CTL_0_OFFSET_ADDR, (0x1 << 8));
	}
}
...
void consys_identify_adie_mt6781_gen(void)
{
	...
	if (mtk_wcn_consys_get_adie_chipid() == SECONDARY_ADIE) {
		CONSYS_REG_WRITE_MASK(conn_reg.mcu_top_misc_on_base +
						CONSYS_GEN_CONN_ON_RSV_OFFSET_ADDR, 0x0, 0x300);
	} else {
		CONSYS_REG_WRITE_MASK(conn_reg.mcu_top_misc_on_base +
						CONSYS_GEN_CONN_ON_RSV_OFFSET_ADDR, 0x100, 0x300);
	}
}
...
void consys_wifi_ctrl_setting_mt6781_gen(void)
{
	...
	if (mtk_wcn_consys_get_adie_chipid() == SECONDARY_ADIE) {
		CONSYS_SET_BIT(vir_addr_consys_gen_gpio_base +
						CONSYS_GEN_GPIO_DIR6_OFFSET_ADDR, (0x1 << 7));
		CONSYS_SET_BIT(vir_addr_consys_gen_gpio_base +
						CONSYS_GEN_GPIO_DOUT6_OFFSET_ADDR, (0x1 << 7));
		CONSYS_REG_WRITE_MASK(vir_addr_consys_gen_gpio_base +
						CONSYS_GEN_GPIO_MODE24_OFFSET_ADDR, 0x0, 0x70000000);
	}
...
void consys_set_mcu_mem_pdn_delay_mt6781_gen(void)
{
	...
	if (!vir_addr_consys_gen_conn_rgu_on_base) {
		pr_err("vir_addr_consys_gen_conn_rgu_on_base(%x) ioremap fail\n", CONSYS_GEN_CONN_RGU_ON_BASE_ADDR);
		return;
	}
...
int consys_polling_goto_idle_mt6781_gen(unsigned int *pconsys_ver_id)
{
	...
	while (consys_ver_id != 0x1d1e) {
		if (cnt > 40) {
			pr_err("can not go into idle loop!\n");
			break;
		}
...
void consys_wifi_ctrl_switch_conn_mode_mt6781_gen(void)
{
	...
	if (mtk_wcn_consys_get_adie_chipid() == SECONDARY_ADIE) {
		CONSYS_REG_WRITE_MASK(vir_addr_consys_gen_gpio_base +
						CONSYS_GEN_GPIO_MODE24_OFFSET_ADDR, 0x10000000, 0x70000000);
	}
