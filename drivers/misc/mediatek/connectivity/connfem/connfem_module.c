...
static int cfm_ioc_is_available_hdlr(unsigned long usr_arg)
{
	...
	if (copy_from_user(&data, (void *)usr_arg, sizeof(data)) != 0) {
		pr_debug("%s, copy_from_user failed", __func__);
		return -EINVAL;
	}

	data.is_available = connfem_is_available(data.fem_type);
	pr_debug("%s, type:%d, return %d",
...
static int cfm_ioc_epa_fn_stat_hdlr(unsigned long usr_arg)
{
	...
	if (data.subsys != CONNFEM_SUBSYS_WIFI &&
		...
		pr_debug("%s, unsupported subsys %d", __func__, data.subsys);
		err = -EINVAL;
		goto fn_stat_done;
	}

	if (!connfem_ctx) {
		pr_debug("[WARN] %s, %d '%s', No ConnFem context",
	...
	pr_debug("%s, %d '%s', return cnt:%d, entry_sz:%d",
		__func__, data.subsys, cfm_subsys_name[data.subsys],
				...
				uint64_t usr_names)
{
	...
	if (!names) {
		pr_debug("%s, no flags, set container size to 0", __func__);
		return cfm_ioc_epa_fn_empty(usr_names);
	}

	if (usr_cnt < names->cnt || usr_entry_sz < names->entry_sz) {
		pr_debug("%s, not enough space, user(%d*%d) < need(%d*%d)",
			__func__,
	...
	if (copy_to_user((void *)usr_names, names, sz) != 0) {
		pr_debug("%s, copy_to_user failed #2", __func__);
		return -EINVAL;
	}
...
static int cfm_ioc_epa_fn_hdlr(unsigned long usr_arg)
{
	...
	if (!data.names) {
		pr_debug("%s, invalid parameter, names is NULL", __func__);
		return -EINVAL;
	}
...
static int cfm_ioc_epa_info_hdlr(unsigned long usr_arg)
{
	...
	if (err < 0) {
		pr_debug("%s, retrieve FEM info failed", __func__);
		return err;
	}
				       ...
				       unsigned long arg)
{
	...
	default:
		pr_debug("Unsupported ioctl cmd 0x%x", cmd);
		err = -EOPNOTSUPP;
		break;
	};
...
static long connfem_dev_compat_ioctl(struct file *filp, unsigned int cmd,
							unsigned long arg)
{
	pr_debug("cmd:0x%x, arg:0x%lx, redirecting to unlocked ioctl", cmd, arg);
...
static int connfem_plat_probe(struct platform_device *pdev)
{
	...
	if (!cfm) {
		pr_debug("Probe, missing platform device data for '%s'",
			pdev->name);
		err = -EOPNOTSUPP;
		goto probe_end;
	}
	...
	cfm->pdev = pdev;
	pr_debug("Probed '%s': 0x%x", pdev->name, cfm->id);

	err = cfm_dt_parse(cfm);
	if (err < 0) {
		pr_debug("%s device tree error %d",
			pdev->name, err);
		 ...
		 */
		if (err == -EAGAIN) {
			pr_debug("Get -EAGAIN and set error to -EPROBE_DEFER");
			err = -EPROBE_DEFER;
			goto probe_end;
		}
	...
	if (connfem_ctx) {
		pr_debug("Failed to register '%s' context, '%s' exists",
			pdev->name,
	...
	pr_debug("ConnFem context '%s' registered, linked to cdev", pdev->name);
...
static int connfem_plat_remove(struct platform_device *pdev)
{
	...
	cfm = (struct connfem_context *)of_device_get_match_data(&pdev->dev);
	if (!cfm) {
		pr_debug("Remove, missing platform device data for '%s'",
			pdev->name);
		return -EOPNOTSUPP;
	}
...
static int __init connfem_mod_init(void)
{
	...
	pr_debug("Internal load: %d", connfem_is_internal());
	...
	if (ret < 0) {
		pr_debug("[WARN] ConnFem platform driver registration failed: %d",
				ret);
	...
	ret = alloc_chrdev_region(&connfem_cdev_ctx.devId, 0, CONNFEM_DEV_NUM,
							  CONNFEM_DRIVER_NAME);
	if (ret < 0) {
		pr_debug("[WARN] ConnFem alloc chrdev region failed: %d", ret);
		ret = -20;
		goto mod_init_err_skip_free;
	}
	connfem_major = MAJOR(connfem_cdev_ctx.devId);
	pr_debug("ConnFem DevID major %d", connfem_major);
	...
	if (IS_ERR(connfem_cdev_ctx.class)) {
		pr_debug("[WARN] ConnFem create class failed");
		ret = -30;
		goto mod_init_err;
	}
	...
	if (!connfem_cdev_ctx.chrdev) {
		pr_debug("[WARN] ConnFem create device failed");
		ret = -40;
		goto mod_init_err;
	}
	...
	ret = cdev_add(&connfem_cdev_ctx.cdev, connfem_cdev_ctx.devId,
				   CONNFEM_DEV_NUM);
	if (ret < 0) {
		pr_debug("[WARN] ConnFem add device failed");
		ret = -50;
		goto mod_init_err;
	}
