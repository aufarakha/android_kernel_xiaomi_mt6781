...
static int mt6357_led_get_clock(struct led_classdev *cdev, int *en)
{
	...
	pr_debug("get RG_DRV_ISINK_CK_PDN[0x%0x]: %d", RG_DRV_ISINK_CK_PDN, *en);
	switch (led->id) {
	case MT6357_ISINK1:
	...
	pr_debug("%s: PMIC LED(%d) Get clock %s.\n",
...
static int mt6357_led_set_clock(struct led_classdev *cdev, int en)
{
	...
	case MT6357_ISINK1:
		...
		pr_debug("set ISINK_EN_CTRL[0x%0x]: 0x%0x",
			RG_DRV_ISINK_CK_PDN,
			value << RG_DRV_ISINK1_CK_PDN_SHIFT);
...
static int mt6357_led_set_ISINK(struct led_classdev *cdev, int en)
{
	...
	case MT6357_ISINK1:
		...
		pr_debug("set ISINK_EN_CTRL[0x%0x]: 0x%0x", ISINK_EN_CTRL, en_value | chop_value);
		break;
	case MT6357_ISINK2:
	case MT6357_ISINK3:
	default:
...
static int mt6357_led_get_ISINK(struct led_classdev *cdev, int *CTL0, int *CTL1)
{
	...
	case MT6357_ISINK1:
		...
		pr_debug("get ISINK_EN_CTRL[0x%0x]: 0x%0x", ISINK_EN_CTRL, value);
...
static int mt6357_led_change_mode(struct led_classdev *cdev, int mode)
{
	...
	case MT6357_ISINK1:
		...
		pr_debug("set ISINK_CH1_MODE[0x%0x]: 0x%0x", ISINK_CH1_MODE, mode_val);
		if (ret < 0) {
			dev_notice(led->parent->dev,
...
static int mt6357_led_get_current_step(struct mt_led_info *info, int *value)
{
	...
	case MT6357_ISINK1:
		...
		pr_debug("get ISINK_CH1_STEP[0x%0x]: 0x%0x", ISINK_CH1_STEP, *value);
...
static int mt6357_led_set_current_step(struct mt_led_info *info, int step)
{
	...
	case MT6357_ISINK1:
		...
		pr_debug("set ISINK_CH1_STEP[0x%0x]: 0x%0x", ISINK_CH1_STEP, value);
		break;
	case MT6357_ISINK2:
	case MT6357_ISINK3:
	default:
...

{
	...
	case MT6357_ISINK1:
		...
		pr_debug("get ISINK_DIM1_DUTY[0x%0x]: 0x%0x", ISINK_DIM1_DUTY, *value);
...

{
	...
	case MT6357_ISINK1:
		...
		pr_debug("set ISINK_DIM1_DUTY[0x%0x]: 0x%0x", ISINK_DIM1_DUTY, value);
		break;
	case MT6357_ISINK2:
	case MT6357_ISINK3:
	default:
...
static int mt6357_led_get_pwm_dim_freq(struct mt_led_info *info, int *value)
{
	...
	case MT6357_ISINK1:
		...
		pr_debug("get ISINK_DIM1_FSEL [0x%0x]: 0x%0x", ISINK_DIM1_FSEL, *value);
...
static int mt6357_led_set_pwm_dim_freq(struct mt_led_info *info, int freq)
{
	...
	case MT6357_ISINK1:
		...
		pr_debug("set ISINK_DIM1_FSEL[0x%0x]: 0x%0x", ISINK_DIM1_FSEL, value);
		if (ret < 0) {
			dev_notice(led->parent->dev, "%s: ISINK_DIM1_FSEL Reg(0x%x) Write ERROR.\n",
				__func__, ISINK_DIM1_FSEL);
			return -1;
...
static int mt6357_led_get_breath_tr1(struct mt_led_info *info, int *value)
{
	...
	case MT6357_ISINK1:
		...
		pr_debug("get ISINK_BREATH1_TR_SEL[0x%0x]1: 0x%0x", ISINK_BREATH1_TR_SEL, *value);
...
static int mt6357_led_get_breath_tf1(struct mt_led_info *info, int *value)
{
	...
	case MT6357_ISINK1:
		...
		pr_debug("get ISINK_BREATH1_TR_SEL[0x%0x]: 0x%0x", ISINK_BREATH1_TR_SEL, *value);
...
static int mt6357_led_get_breath_ton(struct mt_led_info *info, int *value)
{
	...
	case MT6357_ISINK1:
		...
		pr_debug("get ISINK_BREATH1_TON_SEL[0x%0x]: 0x%0x", ISINK_BREATH1_TON_SEL, *value);
...
static int mt6357_led_set_breath_tr1(struct mt_led_info *info, int time)
{
	...
	case MT6357_ISINK1:
		...
		pr_debug("set ISINK_BREATH1_TR_SEL[0x%0x]: 0x%0x", ISINK_BREATH1_TR_SEL, value);
		if (ret < 0) {
...
static int mt6357_led_set_breath_ton(struct mt_led_info *info, int time)
{
	...
	case MT6357_ISINK1:
		...
		pr_debug("set ISINK_BREATH1_TON_SEL[0x%0x]: 0x%0x", ISINK_BREATH1_TON_SEL, value);
		break;
	case MT6357_ISINK2:
	case MT6357_ISINK3:
	default:
				     ...
				     struct device_node *np)
{
	...
	pr_debug("mt6357 parse led[%d]: %s, %s, %s\n",
