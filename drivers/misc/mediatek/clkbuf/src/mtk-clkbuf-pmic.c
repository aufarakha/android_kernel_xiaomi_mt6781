...
static inline struct pmic_clkbuf_dts *find_pmic_dts_node(u32 dts)
{
	...
	pr_debug("[%s]: no %s property, function not support or something wrong\n",
...
static inline void pmic_clkbuf_read(u32 dts, u32 id, u32 *val)
{
	...
	if (!node) {
		pr_debug("%s: pmic node property not stored, something wrong!\n",
				__func__);
		return;
	}
...
static inline void pmic_clkbuf_update(u32 dts, u32 id, u32 val)
{
	...
	if (_is_pmic_clk_buf_debug_enable()) {
		pr_debug("[%s]: val: 0x%x, shift val: 0x%x\n",
...
{
	if (_is_pmic_clk_buf_debug_enable())
		pr_debug("%s: HW BBLPM not support\n", __func__);
}
...
{
	if (_is_pmic_clk_buf_debug_enable())
		pr_debug("%s: Driving current not support\n", __func__);
}
...
{
	if (_is_pmic_clk_buf_debug_enable())
		pr_debug("%s: Dump misc log for clkbuf not support\n", __func__);
	return 0;
}
...
static void mt6357_clk_buf_get_xo_en(u32 *stat)
{
	...
	if (_is_pmic_clk_buf_debug_enable())
		pr_debug("[%s]: idx: %d, AUXOUT write: %u\n",
...
static void mt6359_clk_buf_get_xo_en(u32 *stat)
{
	...
	pr_debug("[%s]: EN_STAT=%u %u %u %u %u %u\n",
		__func__,
		stat[XO_SOC],
		stat[XO_WCN],
		stat[XO_NFC],
					...
					struct pmic_clkbuf_dts *pmic_dts_node)
{
	...
	if (!pmic_dts_node) {
		pr_debug("[%s]: no pmic clkbuf dts node, find mask failed!\n",
				__func__);
		return -1;
	}
					...
					const char *pmic_prop)
{
	...
	if (!(pmic_dts_node->cfg.ofs)) {
		pr_debug("[%s]: allocate cfg offset memory failed\n",
				__func__);
	...
	for (idx = 0; idx < n_prop; idx++) {
		...
		if (ret) {
			pr_debug("[%s]: find %s cfg offset index %u failed\n",
				__func__, pmic_prop, idx);
			goto offset_not_found;
		}

		if (_is_pmic_clk_buf_debug_enable())
			pr_debug("[%s]: find %s cfg offset index %u: %u\n",
					__func__,
					pmic_prop,
					idx,
					pmic_dts_node->cfg.ofs[idx]);
		...
		if (ret) {
			pr_debug("[%s]: find %s cfg bit index %u failed\n",
				__func__, pmic_prop, idx);
			goto bit_not_found;
		}

		if (_is_pmic_clk_buf_debug_enable())
			pr_debug("[%s]: find %s cfg bit index %u: %u\n",
					__func__,
					pmic_prop,
					idx,
					pmic_dts_node->cfg.bit[idx]);
				...
				struct regmap *regmap)
{
	...
	ret = of_property_read_u32(node,
			...
			&n_prop);
	if (ret) {
		pr_debug("[%s]: read number of pmic clkbuf dependent failed\n",
				__func__);
		goto no_property;
	}
	...
	for (idx = 0; idx < n_prop; idx++) {
		...
		if (ret) {
			pr_debug("[%s]: read pmic clkbuf dependent failed\n",
					__func__);
		...
		if (ret) {
			pr_debug("[%s]: find property %s failed\n",
				__func__,
				prop);
