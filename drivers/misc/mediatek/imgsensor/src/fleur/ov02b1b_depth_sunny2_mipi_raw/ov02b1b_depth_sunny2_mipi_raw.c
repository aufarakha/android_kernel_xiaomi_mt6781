...
#include "ov02b1b_depth_sunny2_mipi_raw.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV02B1B_DEPTH_SUNNY2_SENSOR_ID,
...
unsigned int ov02b1b_sunny2_read_otp_info(struct i2c_client *client,
...
static void ov02b1b_sunny2_read_otp(void)
{
...
static int get_ov02b1b_lens_id(void)
{
        int lens_id;
        write_cmos_sensor(0xfd, 0x06);
        lens_id = read_cmos_sensor(0x12);
        return lens_id;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    kal_uint8 lens_id = 0;
        //ov02b1b_sunny2_read_otp();
	vendor_id = get_ov02b1b_vendor_id();
	lens_id = get_ov02b1b_lens_id();
	LOG_INF("zyt__vendor_id:%x, lens_id:%x\n", vendor_id, lens_id);
	if (vendor_id != VENDOR_ID || lens_id != LENS_ID) {
...
static kal_uint32 open(void)
{
	...
	if (lens_id == LENS_ID) {
	    ...
	    LOG_INF("zyt__lens_id:%x setflip HV_MIRROR:%x\n", lens_id, IMAGE_HV_MIRROR);
	}
...
UINT32 OV02B1B_DEPTH_SUNNY2_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
