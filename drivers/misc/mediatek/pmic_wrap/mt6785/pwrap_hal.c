...
static void __iomem *modem_temp_share_base;
...
{
	pr_debug("[PMIC_WRAP] PMIC_WRAP do Nothing.\n");
	return 0;
}
...
#endif
{
	...
	do {
		if (_pwrap_timeout_ns(start_time_ns, timeout_ns)) {
			pr_notice("wait_for_state_ready_init timeout\n");
...
#endif
{
	...
	do {
		if (_pwrap_timeout_ns(start_time_ns, timeout_ns)) {
			pr_notice("wait_for_state_idle timeout\n");
...
#endif
{
	...
	do {
		if (_pwrap_timeout_ns(start_time_ns, timeout_ns)) {
			pr_notice("wait_for_state_ready timeout\n");
	...
	unsigned int wdata, unsigned int *rdata)
{
	...
	WRAP_WR32(PMIC_WRAP_MPU_PMIC_ACC_VIO_INFO_0, 0x20000000);
	WRAP_WR32(PMIC_WRAP_MPU_PWRAP_ACC_VIO_INFO_0, 0x20000000);
	...
	if (return_value != 0) {
		pr_notice("wait_for_fsm_idle fail, ret=%d\n", return_value);
		goto FAIL;
	}
	wacs_write = write << 31;
	wacs_adr = (adr >> 1) << 16;
	...
	if (write == 0) {
		...
		if (return_value != 0) {
			pr_notice("fsm vldclr fail, ret=%d\n", return_value);
			goto FAIL;
		}
	...
	unsigned int wdata, unsigned int *rdata)
{
	...
	if (return_value != 0) {
		pr_notice("wait_fsm_idle fail, ret=%x\n", return_value);
		return return_value;
	}
...
static void __pwrap_spi_clk_set(void)
{
	...
	pr_debug("PMICW_CLOCK_CTRL(before):0x%x\n", WRAP_RD32(PMICW_CLOCK_CTRL));
	WRAP_WR32(PMICW_CLOCK_CTRL_CLR, 0x2);
	pr_debug("PMICW_CLOCK_CTRL(after):0x%x\n", WRAP_RD32(PMICW_CLOCK_CTRL));
...
static void _pwrap_InitStaUpd(void)
{
	...
	WRAP_WR32(PMIC_WRAP_SIG_ADR,
		  (PMIC_EXT_DEW_CRC_VAL_ADDR << 16 | PMIC_DEW_CRC_VAL_ADDR));
#endif
#else
	/* Signature */
#ifndef DUAL_PMICS
	...
	/* MD ADC Interface */
	pr_debug("write MODEM_TEMP_SHARE_CTRL start\n");
	WRAP_WR32(MODEM_TEMP_SHARE_CTRL, 0xf0);
	pr_debug("write MODEM_TEMP_SHARE_CTRL ok\n");
	pr_debug("MODEM_TEMP_SHARE_CTRL:%x\n", WRAP_RD32(MODEM_TEMP_SHARE_CTRL));

	WRAP_WR32(PMIC_WRAP_MD_AUXADC_RDATA_LATEST_ADDR,
		  (PMIC_AUXADC_ADC_OUT_DCXO_BY_AP_ADDR << 16) +
	...
	WRAP_WR32(PMIC_WRAP_INT_GPS_AUXADC_CMD_ADDR,
		  (PMIC_AUXADC_RQST_DCXO_BY_GPS_ADDR << 16) +
		   PMIC_AUXADC_RQST_CH7_BY_GPS_ADDR);
...
static void _pwrap_enable(void)
{
	...
	WRAP_WR32(PMIC_WRAP_INT1_EN, 0x00001fff);
}
...
static signed int _pwrap_init_sistrobe(int dual_si_sample_settings)
{
	...
	for (si_en_sel = 0; si_en_sel < 8; si_en_sel++) {
		for (si_ck_sel = 0; si_ck_sel < 2; si_ck_sel++) {
			...
			if (rdata == DEFAULT_VALUE_READ_TEST) {
				...
				pr_debug("si_en_sel = %x, si_ck_sel = %x\n",
					 si_en_sel, si_ck_sel);
				pr_debug("si_sample_ctrl = %x, rdata = %x\n",
	...
	if (found == 0) {
		result_faulty |= 0x1;
		pr_notice("result_faulty (not found) = %d\n", result_faulty);
	}
	if ((si_en_sel == 7) && (si_ck_sel == 1)) {
		result_faulty |= 0x2;
		pr_notice("result_faulty (last setting) = %d\n", result_faulty);
	}
	...
	pr_debug("si_sample_ctrl=0x%x(before)\n", si_sample_ctrl);
#if defined(CONFIG_FPGA_EARLY_PORTING)
	si_sample_ctrl = si_sample_ctrl + 0x0;
	pr_debug("si_sample_ctrl=0x%x(after)(FPGA)\n", si_sample_ctrl);
#else
	si_sample_ctrl = si_sample_ctrl + 0x20;
	pr_debug("si_sample_ctrl=0x%x(after)(non-FPGA)\n", si_sample_ctrl);
#endif

	WRAP_WR32(PMIC_WRAP_SI_SAMPLE_CTRL, si_sample_ctrl);
	if (si_dly == 10) {
		pr_debug("SI Strobe Calibration For PMIC 0 Done\n");
		pr_debug("si_sample_ctrl = %x, si_dly = %x\n",
...
static int _pwrap_wacs2_write_test(int pmic_no)
{
	...
	if (pmic_no == 0) {
		...
		pwrap_read_nochk(PMIC_DEW_WRITE_TEST_ADDR, &rdata);
		if (rdata != 0xa55a) {
			pr_notice("Err: w_rdata = 0x%x, exp = 0xa55a\n", rdata);
...
signed int pwrap_init(void)
{
	...
	pr_notice("PMIC_WRAP_SI_SAMPLE_CTRL = 0x%x\n",
	...
	pr_debug("_pwrap_lock_SPISLVReg ok\n");
	pr_debug("%s Done!!!!!!!!!\n", __func__);
	...
	pwrap_ut(1);
#endif
	pr_debug("channel pass\n");

#ifdef PMIC_WRAP_MONITOR_SUPPORT

	/* Read Last six command */
	pwrap_read_nochk(PMIC_RECORD_CMD0_ADDR, &rdata);
	pr_notice("REC_CMD0:0x%x (The last adr)\n", (rdata & 0x3fff));
	pwrap_read_nochk(PMIC_RECORD_WDATA0_ADDR, &rdata);
	pr_notice("REC_WDATA0:0x%x (The last command wdata)\n", rdata);

	pwrap_read_nochk(PMIC_RECORD_CMD1_ADDR, &rdata);
	pr_notice("REC_CMD1:0x%x (The second-last adr)\n", (rdata & 0x3fff));
	pwrap_read_nochk(PMIC_RECORD_WDATA1_ADDR, &rdata);
	pr_notice("REC_WDATA1:0x%x (The second-last command wdata)\n", rdata);

	pwrap_read_nochk(PMIC_RECORD_CMD2_ADDR, &rdata);
	pr_notice("REC_CMD2:0x%x (The third-last adr)\n", (rdata & 0x3fff));
	pwrap_read_nochk(PMIC_RECORD_WDATA2_ADDR, &rdata);
	pr_notice("REC_WDATA2:0x%x (The third-last command wdata)\n", rdata);

	pwrap_read_nochk(PMIC_RECORD_CMD3_ADDR, &rdata);
	pr_notice("REC_CMD3:0x%x (The fourth-last addr)\n", (rdata & 0x3fff));
	pwrap_read_nochk(PMIC_RECORD_WDATA3_ADDR, &rdata);
	pr_notice("REC_WDATA3:0x%x (The fourth-last command wdata)\n", rdata);

	pwrap_read_nochk(PMIC_RECORD_CMD4_ADDR, &rdata);
	pr_notice("REC_CMD4:0x%x (The fifth-last addr)\n", (rdata & 0x3fff));
	pwrap_read_nochk(PMIC_RECORD_WDATA4_ADDR, &rdata);
	pr_notice("REC_WDATA4:0x%x (The fifth-last command wdata)\n", rdata);

	pwrap_read_nochk(PMIC_RECORD_CMD5_ADDR, &rdata);
	pr_notice("REC_CMD5:0x%x (The sixth-last addr)\n", (rdata & 0x3fff));
	pwrap_read_nochk(PMIC_RECORD_WDATA5_ADDR, &rdata);
	pr_notice("REC_WDATA5:0x%x (The sixth-last command wdata)\n", rdata);
	...
	pr_notice("PMIC_WRAP_MONITOR_CTRL = 0x%x\n",
		   WRAP_RD32(PMIC_WRAP_MONITOR_CTRL));
	pr_notice("PMIC_WRAP_MONITOR_TARGET_CHANNEL_0 = 0x%x\n",
		   WRAP_RD32(PMIC_WRAP_MONITOR_TARGET_CHANNEL_0));
	pr_notice("PMIC_WRAP_MONITOR_TARGET_ADR_0 = 0x%x\n",
		   WRAP_RD32(PMIC_WRAP_MONITOR_TARGET_ADR_0));
	pr_notice("PMIC_WRAP_MONITOR_TARGET_WDATA_0 = 0x%x\n",
		   WRAP_RD32(PMIC_WRAP_MONITOR_TARGET_WDATA_0));
#else
	/* enable logging mode */
	pr_debug("PMIC_WRAP_MONITOR_MODE = Logging Mode\n");
	...
	pr_debug("PMIC_WRAP_MONITOR_CTRL = 0x%x\n",
		 WRAP_RD32(PMIC_WRAP_MONITOR_CTRL));
	pr_debug("PMIC_WRAP_MONITOR_TARGET_CHANNEL_0 = 0x%x\n",
		 WRAP_RD32(PMIC_WRAP_MONITOR_TARGET_CHANNEL_0));
	pr_debug("PMIC_WRAP_MONITOR_TARGET_ADR_0 = 0x%x\n",
		 WRAP_RD32(PMIC_WRAP_MONITOR_TARGET_ADR_0));
	pr_debug("PMIC_WRAP_MONITOR_TARGET_WDATA_0 = 0x%x\n",
		 WRAP_RD32(PMIC_WRAP_MONITOR_TARGET_WDATA_0));
#endif
...
static inline void pwrap_dump_ap_register(void)
{
	...
	WRAP_WR32(PMIC_WRAP_MONITOR_CTRL, 0x405); /* reenable */
#else
	/* Matching mode and Continue recording after interrupt trigger */
	WRAP_WR32(PMIC_WRAP_MONITOR_CTRL, 0x5); /* reenable */
#endif
	WRAP_WR32(PMIC_WRAP_WACS2_EN, 0x1);
...
static inline void pwrap_dump_pmic_register(void)
{
	...
	if (__ratelimit(&ratelimit)) {
		...
		for (i = 0; i <= 14; i++) {
			reg_addr = (PMIC_RG_SPI_MISO_MODE_SEL_ADDR + i * 2);
			ret = pwrap_read_nochk(reg_addr, &reg_value);
			pr_notice("[REG]0x%x=0x%x\n", reg_addr, reg_value);
		}
	}
...
static void pwrap_logging_at_isr(void)
{
	...
	if (__ratelimit(&ratelimit)) {
		...
		pr_notice("MONITOR_CTRL=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_MONITOR_CTRL));
		pr_notice("MONITOR_TARGET_CHANNEL_0=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_MONITOR_TARGET_CHANNEL_0));
		pr_notice("MONITOR_TARGET_ADR_0=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_MONITOR_TARGET_ADR_0));
		pr_notice("MONITOR_TARGET_WDATA_0=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_MONITOR_TARGET_WDATA_0));
		...
		pr_notice("CH_SEQ_4=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_CHANNEL_SEQUENCE_4));
		pr_notice("CH_SEQ_5=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_CHANNEL_SEQUENCE_5));
		pr_notice("CH_SEQ_6=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_CHANNEL_SEQUENCE_6));
		pr_notice("CH_SEQ_7=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_CHANNEL_SEQUENCE_7));
		pr_notice("ADR_SEQ_0=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_0));
		pr_notice("ADR_SEQ_1=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_1));
		pr_notice("ADR_SEQ_2=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_2));
		pr_notice("ADR_SEQ_3=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_3));
		pr_notice("ADR_SEQ_4=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_4));
		pr_notice("ADR_SEQ_5=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_5));
		pr_notice("ADR_SEQ_6=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_6));
		pr_notice("ADR_SEQ_7=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_7));
		pr_notice("ADR_SEQ_8=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_8));
		pr_notice("ADR_SEQ_9=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_9));
		pr_notice("ADR_SEQ_10=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_10));
		pr_notice("ADR_SEQ_11=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_11));
		pr_notice("ADR_SEQ_12=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_12));
		pr_notice("ADR_SEQ_13=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_13));
		pr_notice("ADR_SEQ_14=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_14));
		pr_notice("ADR_SEQ_15=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_ADR_SEQUENCE_15));
		...
		pr_notice("WDATA_SEQ_8=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_WDATA_SEQUENCE_8));
		pr_notice("WDATA_SEQ_9=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_WDATA_SEQUENCE_9));
		pr_notice("WDATA_SEQ10=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_WDATA_SEQUENCE_10));
		pr_notice("WDATA_SEQ11=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_WDATA_SEQUENCE_11));
		pr_notice("WDATA_SEQ12=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_WDATA_SEQUENCE_12));
		pr_notice("WDATA_SEQ13=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_WDATA_SEQUENCE_13));
		pr_notice("WDATA_SEQ14=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_WDATA_SEQUENCE_14));
		pr_notice("WDATA_SEQ15=0x%x\n",
			  WRAP_RD32(PMIC_WRAP_WDATA_SEQUENCE_15));
		...
		WRAP_WR32(PMIC_WRAP_MONITOR_CTRL, 0x405); /* reenable */
#else
		/* Matching mode and Continue record after interrupt trigger */
		WRAP_WR32(PMIC_WRAP_MONITOR_CTRL, 0x5); /* reenable */
#endif
...
static int pwrap_of_iomap(void)
{
	 ...
	 * INFRACFG_AO, TOPCKGEN, SCP_CLK_CTRL, MODEM_TEMP_SHARE
	 */

	struct device_node *infracfg_ao_node;
	struct device_node *topckgen_node;
	struct device_node *modem_temp_share_node;
	...
	modem_temp_share_node =
		of_find_compatible_node(NULL, NULL,
		"mediatek,modem_temp_share");
	if (!modem_temp_share_node) {
		pr_notice("get MODEM_TEMP_SHARE failed\n");
		return -ENODEV;
	}

	modem_temp_share_base = of_iomap(modem_temp_share_node, 0);
	if (!modem_temp_share_base) {
		pr_notice("MODEM_TEMP_SHARE iomap failed\n");
		return -ENOMEM;
	}
...
static irqreturn_t mt_pmic_wrap_irq(int irqno, void *dev_id)
{
	...
	if ((int1_flg & 0xffffffff) != 0) {
		...
		if ((int1_flg & (0x3 << 11)) != 0) {
			pr_notice("[PWRAP] MPU Access Violation\n");
			pwrap_mpu_info();
			pwrap_logging_at_isr();
			pwrap_reenable_pmic_logging();
			...
			pwrap_dump_all_register();
			WRAP_WR32(PMIC_WRAP_INT1_CLR, 0xffffe7ff);
		}
	}
	...
	if ((int0_flg & 0x01) == 0x01) {
		...
		if (ret != 0)
			pr_notice("reset crc fail, ret=%x\n", ret);
...
static void pwrap_int_test(void)
{
	...
	while (1) {
		...
		pr_debug("PMIC_WRAP_EINT_STA=0x%x, INT_STA[0x0452]=0x%x\n",
			 rdata1, rdata2);
		msleep(500);
	}
}
