...
#define MFALSE              0
	...
	#define gSVBW_LIST(module) ({ \
		...
		if (module >= ISP_IRQ_TYPE_INT_CAMSV_START_ST && \
			...
			ptr = &_gSVBW_LIST \
				[module - ISP_IRQ_TYPE_INT_CAMSV_START_ST]; \
		} \
		else { \
	...
	struct mm_qos_request _gSV_BW_REQ[ISP_IRQ_TYPE_INT_CAMSV_END_ST + 1 -
	...
	#define gSV_BW_REQ(module, port) ({ \
		...
		if (module >= ISP_IRQ_TYPE_INT_CAMSV_START_ST && \
			...
			ptr = &_gSV_BW_REQ\
				[module - \
				ISP_IRQ_TYPE_INT_CAMSV_START_ST][port];\
		} \
		else { \
			LOG_NOTICE("sv idx violation , force to ke\n"); \
		} \
