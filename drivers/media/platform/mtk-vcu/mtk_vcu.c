...
static int vcu_gce_set_inst_id(void *ctx, u64 gce_handle)
{
	...
	mutex_unlock(&vcu_ptr->vcu_share);
	pr_debug("[VCU] %s fail ctx %p hndl %llu\n",
	...
	u64 addr, u64 length, bool null_err)
{
	...
	if (null_err)
		pr_debug("%s addr %llx length %llx not found!\n", __func__, addr, length);
	else
	...
	unsigned char cmd, u64 addr, u64 data, u32 mask, u32 gpr, u32 dma_offset, u32 dma_size)
{
	...
	case CMD_READ:
		if (vcu_check_reg_base(vcu, addr, 4) == 0)
			...
			pr_debug("[VCU] CMD_READ wrong addr: 0x%llx\n", addr);
	break;
	case CMD_WRITE:
		if (vcu_check_reg_base(vcu, addr, 4) == 0)
			cmdq_pkt_write(pkt, vcu->clt_base, addr, data, mask);
		else
			pr_debug("[VCU] CMD_WRITE wrong addr: 0x%llx 0x%llx 0x%x\n",
				addr, data, mask);
	break;
	...
	case CMD_SEC_WRITE:
		...
		if (vcu_check_reg_base(vcu, addr, 4) == 0) {
			...
			pr_debug("[VCU] CMD_SEC_WRITE wrong addr: 0x%x 0x%x 0x%x 0x%x\n",
	...
	case CMD_POLL_REG:
		if (vcu_check_reg_base(vcu, addr, 4) == 0)
			...
			pr_debug("[VCU] CMD_POLL_REG wrong addr: 0x%llx 0x%llx 0x%x\n",
				addr, data, mask);
	break;
	case CMD_WAIT_EVENT:
	...
	case CMD_MEM_MV:
		...
		if ((reg_check == 0 || src_page != NULL) && dst_page != NULL) {
			...
			pr_debug("[VCU] CMD_MEM_MV wrong addr/data: 0x%llx 0x%llx\n",
				addr, data);
...
static void vcu_gce_flush_callback(struct cmdq_cb_data data)
{
	...
	if (j < 0) {
		pr_debug("[VCU] flush_callback get_inst_id fail!!%d\n", j);
		return;
	}
	...
	struct mtk_vcu_queue *q, unsigned long arg)
{
	...
	if (ret != 0L) {
		pr_debug("[VCU] %s(%d) gce_cmdq_obj copy_from_user failed!%d\n",
	...
	if (buff.cmdq_buff.cmds_user_ptr > 0) {
		...
		if (ret != 0L) {
			pr_debug("[VCU] %s(%d) gce_cmds copy_from_user failed!%d\n",
				__func__, __LINE__, ret);
	...
	if (buff.cmdq_buff.codec_type >= VCU_CODEC_MAX ||
		...
		pr_debug("[VCU] %s invalid core(th) id %d\n",
			__func__, core_id);
		mutex_unlock(&vcu->gce_cmds_mutex[codec_type]);
		return -EINVAL;
	}
	...
	if (cl == NULL) {
		pr_debug("[VCU] %s gce thread is null id %d type %d\n",
			__func__, core_id,
	...
	if (IS_ERR_OR_NULL(pkt_ptr)) {
		pr_debug("[VCU] cmdq_pkt_create fail\n");
		pkt_ptr = NULL;
	}
	...
	if (ret < 0) {
		pr_debug("[VCU] cmdq flush fail pkt %p\n", pkt_ptr);
...
static int vcu_wait_gce_callback(struct mtk_vcu *vcu, unsigned long arg)
{
	...
	if (ret != 0L) {
		pr_debug("[VCU] %s(%d) copy_from_user failed!%d\n",
			__func__, __LINE__, ret);
		return -EINVAL;
	}
...
void vcu_get_gce_lock(struct platform_device *pdev, unsigned long codec_type)
{
	...
	if (pdev == NULL) {
		pr_debug("[VCU] %s platform device is null.\n", __func__);
		return;
	}
	if (codec_type >= VCU_CODEC_MAX) {
		pr_debug("[VCU] %s invalid codec type %ld.\n", __func__, codec_type);
		return;
	}
	vcu = platform_get_drvdata(pdev);
	mutex_lock(&vcu->vcu_gce_mutex[codec_type]);
...
static void mtk_vcu_page_vm_close(struct vm_area_struct *vma)
{
	...
	if (atomic_read(&vcu_page->ref_cnt) > 0)
		...
		pr_debug("[VCU][Error] %s ummap fail\n", __func__);
	...
	case VCU_PA_ALLOCATION:
		...
		if (IS_ERR_OR_NULL(mem_priv) == true) {
			...
			ret = (long)copy_to_user(user_data_addr,
				&mem_buff_data,
				(unsigned long)sizeof(struct mem_obj));
			pr_debug("[VCU] ALLOCATION %d failed!\n", cmd == VCU_MVA_ALLOCATION);
