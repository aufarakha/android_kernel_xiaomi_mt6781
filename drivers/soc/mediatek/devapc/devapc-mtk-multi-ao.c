...
static void sramrom_vio_handler(void)
{
	...
	if (sramrom_vio == SRAM_VIOLATION)
		...
		pr_debug(PFX "sramrom_vio:0x%x, sramrom_vio_sta:0x%zx, vio_addr:0x%x\n",
				sramrom_vio,
				sramrom_vio_sta,
				vio_info->vio_addr);
		...
		struct mtk_devapc_soc *soc)
{
	...
	if (IS_ERR(mtk_devapc_ctx->devapc_infra_clk))
		pr_debug(PFX "(Infra) Cannot get devapc clock from CCF (%d)\n",
	...
	if (unlikely(mtk_devapc_ctx->sramrom_base == NULL))
		pr_debug(PFX "parse sramrom_base failed\n");
