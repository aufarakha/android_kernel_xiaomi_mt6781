...
#define ISP_PWR_CON		SPM_REG(0x334)
#define ISP2_PWR_CON		SPM_REG(0x338)
#define IPE_PWR_CON		SPM_REG(0x33C)
#define VDE_PWR_CON		SPM_REG(0x340)
#define VEN_PWR_CON		SPM_REG(0x348)
#define DIS_PWR_CON		SPM_REG(0x354)
#define AUDIO_PWR_CON		SPM_REG(0x358)
...
#define MD1_PROT_STEP2_0_MASK            ((0x1 << 0) \
	|(0x1 << 10) \
	|(0x1 << 16))
#define MD1_PROT_STEP2_0_ACK_MASK        ((0x1 << 0) \
	|(0x1 << 10) \
	|(0x1 << 16))
...
#define IFR_PROT_STEP1_0_MASK            ((0x1 << 7) \
	|(0x1 << 9) \
	|(0x1 << 12) \
	|(0x1 << 13) \
	|(0x1 << 16) \
	...
	|(0x1 << 24) \
	|(0x1 << 28))
#define IFR_PROT_STEP1_0_ACK_MASK        ((0x1 << 7) \
	|(0x1 << 9) \
	|(0x1 << 12) \
	|(0x1 << 13) \
	|(0x1 << 16) \
...
#define IFR_PROT_STEP1_3_MASK            ((0x1 << 2))
#define IFR_PROT_STEP1_3_ACK_MASK        ((0x1 << 2))
...
#define IFR_PROT_STEP2_3_MASK            ((0x1 << 6) \
	|(0x1 << 16) \
	|(0x1 << 17) \
	|(0x1 << 18) \
	|(0x1 << 24))
#define IFR_PROT_STEP2_3_ACK_MASK        ((0x1 << 6) \
	|(0x1 << 16) \
	|(0x1 << 17) \
	|(0x1 << 18) \
	|(0x1 << 24))
#define DRAMC_MD32_PROT_STEP1_0_MASK     ((0x1 << 24))
#define DRAMC_MD32_PROT_STEP1_0_ACK_MASK   ((0x1 << 24))
...
#define CAM_PROT_STEP2_1_ACK_MASK        ((0x1 << 1) \
	|(0x1 << 3))
#define CAM_PROT_STEP2_2_MASK            ((0x1 << 15))
#define CAM_PROT_STEP2_2_ACK_MASK        ((0x1 << 15))
#define MCUPM_PROT_STEP1_0_MASK          ((0x1 << 27) \
	|(0x1 << 30))
#define MCUPM_PROT_STEP1_0_ACK_MASK      ((0x1 << 27) \
	|(0x1 << 30))
#define MCUPM_PROT_STEP2_0_MASK          ((0x1 << 6))
#define MCUPM_PROT_STEP2_0_ACK_MASK      ((0x1 << 6))
...
#define DRAMC_MD32_PWR_STA_MASK          (0x1 << 12)
...
#define VDE2_PWR_STA_MASK                (0x1 << 17)
#define VEN_PWR_STA_MASK                 (0x1 << 18)
#define VEN_CORE1_PWR_STA_MASK           (0x1 << 19)
#define MDP_PWR_STA_MASK                 (0x1 << 20)
#define DIS_PWR_STA_MASK                 (0x1 << 21)
#define AUDIO_PWR_STA_MASK               (0x1 << 22)
...
#define DRAMC_MD32_SRAM_PDN              (0x1 << 8)
#define DRAMC_MD32_SRAM_PDN_ACK          (0x1 << 12)
#define DRAMC_MD32_SRAM_PDN_ACK_BIT0     (0x1 << 12)
...
{
	/*pr_debug("[CCF] %s\r\n", __func__); */
	return 0;//spm_mtcmos_ctrl_mfg5_bus_prot(STA_POWER_ON);
}

static int MFG5_sys_enable_op(struct subsys *sys)
{
	/*pr_debug("[CCF] %s\r\n", __func__); */
	return 0;//spm_mtcmos_ctrl_mfg5_pwr(STA_POWER_ON);
}

static int MFG5_sys_unprepare_op(struct subsys *sys)
{
	/*pr_debug("[CCF] %s\r\n", __func__); */
	return 0;//spm_mtcmos_ctrl_mfg5_bus_prot(STA_POWER_DOWN);
}

static int MFG5_sys_disable_op(struct subsys *sys)
{
	/*pr_debug("[CCF] %s\r\n", __func__); */
	return 0;//spm_mtcmos_ctrl_mfg5_pwr(STA_POWER_DOWN);
}
...
{
	/*pr_debug("[CCF] %s\r\n", __func__); */
	return 0;//spm_mtcmos_ctrl_adsp_dormant_bus_prot(STA_POWER_ON);
}

static int ADSP_sys_enable_op(struct subsys *sys)
{
	/*pr_debug("[CCF] %s\r\n", __func__); */
	return 0;//spm_mtcmos_ctrl_adsp_dormant_pwr(STA_POWER_ON);
}

static int ADSP_sys_unprepare_op(struct subsys *sys)
{
	/*pr_debug("[CCF] %s\r\n", __func__); */
	return 0;//spm_mtcmos_ctrl_adsp_dormant_bus_prot(STA_POWER_DOWN);
}

static int ADSP_sys_disable_op(struct subsys *sys)
{
	/*pr_debug("[CCF] %s\r\n", __func__); */
	return 0;//spm_mtcmos_ctrl_adsp_dormant_pwr(STA_POWER_DOWN);
}
...
{
	/*pr_debug("[CCF] %s\r\n", __func__); */
	return 0;//spm_mtcmos_ctrl_vpu_pwr(STA_POWER_ON);
}

static int VPU_sys_disable_op(struct subsys *sys)
{
	/*pr_debug("[CCF] %s\r\n", __func__); */
	return 0;//spm_mtcmos_ctrl_vpu_pwr(STA_POWER_DOWN);
}
...
static int clk_mt6833_scpsys_probe(struct platform_device *pdev)
{
...
static const struct of_device_id of_match_clk_mt6833_scpsys[] = {
	{ .compatible = "mediatek,mt6833-scpsys", },
	{}
};

static struct platform_driver clk_mt6833_scpsys_drv = {
	.probe = clk_mt6833_scpsys_probe,
	.driver = {
		.name = "clk-mt6833-scpsys",
		.of_match_table = of_match_clk_mt6833_scpsys,
	},
};
static int __init clk_mt6833_scpsys_init(void)
{
