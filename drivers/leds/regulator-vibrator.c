	...
	unsigned long event, void *data)
{
	...
	case REGULATOR_EVENT_FAIL:
		pr_debug("get regulator oc event: %lu", event);
...
static int vib_probe(struct platform_device *pdev)
{
	...
	if (m_vibr->vibr_conf.reg == NULL) {
		...
		if (devm_regulator_register_notifier(m_vibr->vibr_conf.reg,
			&m_vibr->oc_handle))
			pr_debug("regulator notifier request failed\n");
	}
	...
	if (ret < 0) {
		pr_debug("set voltage for regulator fail\n");
		goto err;
	}
...
static void vib_shutdown(struct platform_device *pdev)
{
	...
	if (vibr->vibr_state) {
		vibr->vibr_state = 0;
		spin_unlock_irqrestore(&vibr->vibr_lock, flags);
		pr_debug("vibrator will disable!\n");
		vibr_disable(vibr);
	} else {
		spin_unlock_irqrestore(&vibr->vibr_lock, flags);
	}
