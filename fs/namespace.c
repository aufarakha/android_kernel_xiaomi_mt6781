				...
				void *data)
{
	...
	put_fs_context(fc);
	#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
    if (!IS_ERR(mnt) && susfs_is_current_zygote_domain()) {
        struct mount *rmnt = real_mount(mnt);

        rmnt->mnt.android_kabi_reserved4 = rmnt->mnt_id;
        rmnt->mnt_id = current->android_kabi_reserved8++;
    }
	#endif
	return mnt;
}
